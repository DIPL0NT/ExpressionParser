...
OperandVec *evaluate_ExpressionTree(ExpressionTreeNode *tree){
	...
	for (int j=0;j<tree->args.count;j++){
		if (cur->treeNode->token.type==OPERAND){
			result->values[i++] = ((Operand*)cur->treeNode->token.data)->value ;
		}
		else if (cur->treeNode->token.type==OPERATOR){
		...
		else if (cur->treeNode->token.type==EXPRESSIONTREENODE_LIST){ //should be the only other case
	...
	if (tree->token.type==EXPRESSIONTREENODE_LIST){
		return result;
	}

	if (tree->token.type==OPERATOR){ //should be the only remaning case
